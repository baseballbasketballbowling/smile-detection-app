...
// 乾杯検知の誤作動防止ゲート
const KANPAI_MIN_FRAMES = 2;   // バッチ内でこのフレーム数以上 kanpai 判定が必要
...
async function analyzeBatch(batch) {
  ...
  content.push({
    ...
    text: [
      ...
      `Be strict: if a frame is blurry, shows no clear human face, or you are unsure, use score 0.0, faces 0, kanpai false.`,
...
function onBatchResult(scores, smilingCounts, faceCounts, kanpaiFlags, obstructedFlags, batch) {
  ...
  // 顔が検出されていないフレームのスコアは信用しない（幻覚スコアでの誤発防止）
  const gatedScores = scores.map((s, i) => (faceCounts[i] ?? 0) >= 1 ? s : 0);
  ...
  try {
    const kanpaiCount = Array.isArray(kanpaiFlags)
      ...
      : 0;
    if (!isInCooldown && kanpaiCount >= KANPAI_MIN_FRAMES) {
      const kanpaiIdx = kanpaiFlags.findIndex((k, i) =>
        k === true
        && (faceCounts[i] ?? 0) >= KANPAI_MIN_FACES
        && (gatedScores[i] ?? 0) >= KANPAI_MIN_SCORE);
      if (kanpaiIdx >= 0) {
        const kScore   = gatedScores[kanpaiIdx];
